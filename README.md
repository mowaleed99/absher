...
5. **بيانات الدخول الافتراضية للمشرف (Default Admin):**
   * يمكن العثور على بيانات المسؤول الافتراضية أو تعديلها من خلال ملف التهيئة الرئيسي لقاعدة البيانات `backend_php/schema.sql` (تأكد من تغيير كلمة المرور الافتراضية فور تثبيت النظام في البيئة الإنتاجية).
