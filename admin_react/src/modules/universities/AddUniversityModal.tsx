...
export function AddUniversityModal({ isOpen, onClose, onSubmit, showToast }: AddUniversityModalProps) {
  const { t } = useI18n();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        showToast(t('msg.uni_added'), 'success');
        setNameAr('');
        setNameEn('');
        onClose();
      } else {
        showToast(res.error || t('msg.error_add_uni'), 'error');
      }
    } finally {
