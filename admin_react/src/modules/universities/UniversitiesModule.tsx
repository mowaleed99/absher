...
export function UniversitiesModule() {
  const { t, lang } = useI18n();
  const isRtl = lang === 'ar';
  const { confirm } = useConfirmDialog();
  const { showToast } = useToast();
  const { universities, isLoading, error, refetch, addUniversity, updateUniversity, deleteUniversity } = useUniversities();
  ...
  const handleDelete = async (university: University) => {
    const confirmed = await confirm({
      title: t('dialog.delete_uni_title'),
      message: `${t('dialog.delete_uni_msg')} (${university.name_ar || university.name})`,
    ...
    if (res.success) {
      showToast(t('msg.uni_deleted'), 'success');
    } else {
      showToast(res.error || t('msg.error_delete_uni'), 'error');
    }
  };
    ...
    <section className="tab-pane active" style={{ padding: '0 4px' }}>
      ...
      <div className="section-header" style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div className="section-title">
          <h2>
            <i className="fa-solid fa-graduation-cap" style={{ color: 'var(--primary)', marginLeft: '8px' }}></i>
        ...
        >
          ...
          <span>{t('universities.add_button')}</span>
        </button>
      </div>
      ...
      >
        ...
        >
          {t('universities.count', { count: filteredUniversities.length })}
        </div>
      </div>
      ...
      {!isLoading && !error && filteredUniversities.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
          <i className="fa-solid fa-graduation-cap fa-3x" style={{ color: 'var(--text-muted)', opacity: 0.4, marginBottom: '16px' }}></i>
          <h3 style={{ fontSize: '1.2rem', marginBottom: '8px' }}>{t('universities.empty_state')}</h3>
          <button
            type="button"
          ...
          >
            <i className="fa-solid fa-plus"></i> {t('universities.add_button')}
          </button>
        </div>
      )}
