...
export function EditDistrictModal({ isOpen, district, onClose, onSubmit, showToast }: EditDistrictModalProps) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    if (district) {
      setNameAr(district.name_ar || district.name || '');
      setNameEn(district.name_en || '');
    }
  }, [district]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        showToast(t('msg.district_updated'), 'success');
        onClose();
      } else {
        showToast(res.error || t('msg.error_update_district'), 'error');
      }
    } finally {
