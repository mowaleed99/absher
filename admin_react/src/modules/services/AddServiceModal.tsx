...
export function AddServiceModal({ isOpen, onClose, onSubmit, showToast }: AddServiceModalProps) {
  const { t } = useI18n();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        showToast(t('msg.service_added'), 'success');
        onClose();
      } else {
        showToast(res.error || t('msg.error_add_service'), 'error');
      }
    } finally {
        ...
        <form onSubmit={handleSubmit} style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="وصف تفصيلي للخدمة ومزاياها وكيفية التقديم عليها..."
              disabled={isSubmitting}
              style={{
                width: '100%',
                padding: '12px 14px',
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Detailed description of the service and procedure..."
              disabled={isSubmitting}
              style={{
                width: '100%',
                padding: '12px 14px',
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  <span>{t('form.add_service_btn')}</span>
                </>
              )}
            </button>
          </div>
