...
interface AddHousingOfferModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (data: HousingOfferFormInput) => Promise<unknown>;
}
...
export function AddHousingOfferModal({ isOpen, onClose, onAdd }: AddHousingOfferModalProps) {
  ...
  // When apartment changes, pre-populate original price if not already modified
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: HousingOfferFormInput = {
      ...
      await onAdd(payload);
      showToast('تمت إضافة عرض السكن الحصري بنجاح', 'success');
      onClose();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'فشل إضافة عرض السكن';
      showToast(msg, 'error');
    } finally {
      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: '#1e293b', padding: '14px', borderRadius: '10px', border: '1px solid #334155' }}>
            ...
            >
              <option value={0}>-- اختر الشقة المراد إنشاء عرض عليها --</option>
              {apartments.map((apt) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px' }}>
            <div>
              ...
              <input
                ...
                placeholder="مثال: عرض بداية الفصل الدراسي - خصم 15%"
                required
                style={{
                  width: '100%',
                  padding: '9px 12px',
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Semester Start Special Offer - 15% OFF"
                style={{
                  width: '100%',
                  padding: '9px 12px',
                  background: '#0d1527',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px' }}>
            <div>
              ...
              <textarea
                ...
                placeholder="تفاصيل العرض، الشروط، الخدمات المشمولة..."
                required
                style={{
                  width: '100%',
                  padding: '9px 12px',
          ...
          >
            ...
            />
            <label htmlFor="is_active_checkbox" style={{ fontSize: '0.88rem', fontWeight: 600, color: '#f8fafc', cursor: 'pointer' }}>
              تفعيل العرض فوراً وإظهاره للطلاب في التطبيق
            </label>
          </div>
