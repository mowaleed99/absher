...
import { PromoFormInput, DiscountType, ScopeType, PromoStatus } from '../../types/promo';
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-container" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '750px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div className="modal-header">
          <h3>
            <i className="fa-solid fa-tag" style={{ color: '#38bdf8', marginLeft: lang === 'ar' ? '8px' : 0, marginRight: lang === 'en' ? '8px' : 0 }}></i>
            {t('promo.add_button')}
          </h3>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600 }}>حالة الكود الابتدائية</label>
            <select className="input-field" value={status} onChange={(e) => setStatus(e.target.value as PromoStatus)}>
              <option value="active">نشط ومتاح فوراً (Active)</option>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '1rem' }}>
            ...
            <button type="submit" className="btn-primary" disabled={isSubmitting}>
              {isSubmitting ? (
                ...
                <span><i className="fa-solid fa-check"></i> حفظ وإنشاء الكود</span>
              )}
            </button>
          </div>
        </form>
