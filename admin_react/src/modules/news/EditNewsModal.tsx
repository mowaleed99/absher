...
export function EditNewsModal({ isOpen, newsItem, onClose, onSubmit, showToast }: EditNewsModalProps) {
  ...
  useEffect(() => {
    if (newsItem) {
      setTitleAr(newsItem.title_ar || newsItem.title || '');
      setTitleEn(newsItem.title_en || '');
      setContentAr(newsItem.content_ar || newsItem.content || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await onSubmit(newsItem.id, {
      ...
      if (res.success) {
        showToast(isRtl ? 'تم تعديل الخبر بنجاح' : 'News updated successfully', 'success');
        onClose();
      } else {
        showToast(res.error || (isRtl ? 'فشل تعديل الخبر' : 'Failed to update news'), 'error');
      }
    } finally {
