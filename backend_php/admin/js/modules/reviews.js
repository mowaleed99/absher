...
export function renderReviews() {
    ...
    if (searchPhrase) {
        ...
        list = list.filter(rev => {
            ...
            if (!rev.service_request_id && !rev.student_id) {
                serviceTitle = window.t('reviews.cs_chat_testimonial');
                sourceLabelText = 'testimonial ' + window.t('reviews.source.testimonial');
            } else {
                sourceLabelText = 'service review ' + window.t('reviews.source.service');
                if (req) {
                    serviceTitle = req.service_title;
                } else if (rev.service_request_id) {
                    serviceTitle = `Request #${rev.service_request_id}`;
                } else {
                    serviceTitle = window.t('reviews.general_testimonial');
                }
            }
    ...
    container.innerHTML = list.map(rev => {
        ...
        let statusText = window.t('reviews.status.pending');
        if (rev.status === 'approved') {
            statusClass = 'status-done';
            statusText = window.t('reviews.status.approved');
        } else if (rev.status === 'rejected') {
            statusClass = 'status-pending'; // fallback style
            statusText = window.t('reviews.status.rejected');
        }
        ...
        const sourceLabel = isLegacyTestimonial
            ? `<span class="badge" style="background: rgba(139, 92, 246, 0.15); color: #8b5cf6; font-size: 0.72rem; padding: 2px 6px; border-radius: 4px; display: inline-block; margin-bottom: 4px; font-weight: bold;">${window.t('reviews.source.testimonial')}</span>`
            : `<span class="badge" style="background: rgba(59, 130, 246, 0.15); color: #3b82f6; font-size: 0.72rem; padding: 2px 6px; border-radius: 4px; display: inline-block; margin-bottom: 4px; font-weight: bold;">${window.t('reviews.source.service')}</span>`;
            ...
            <tr id="review-row-${rev.id}" style="${isProcessing ? 'opacity: 0.6;' : ''}">
                ...
                <td><span style="font-size: 0.85rem; color: var(--text-muted);">${
                    rev.uni && !/^\d+$/.test(rev.uni.trim())
                        ? rev.uni
                        : `<em style="color: var(--text-muted); opacity: 0.6;">${window.t('reviews.field.unavailable')}</em>`
                }</span></td>
                <td>
                    ${sourceLabel}<br/>
                        ...
                        ` : `
                            ${!isApproved ? `
                                <button class="btn btn-primary" style="padding: 6px 12px; font-size: 0.8rem; background: var(--accent-green);" 
                                    ...
                                    <i class="fa-solid fa-check"></i> ${window.t('buttons.accept')}
                                </button>
                            ` : ''}
                            ${!isRejected ? `
                                <button class="btn btn-secondary" style="padding: 6px 12px; font-size: 0.8rem; background: var(--accent-amber); color: black;" 
                                    ...
                                    <i class="fa-solid fa-ban"></i> ${window.t('buttons.reject')}
                                </button>
                            ` : ''}
...
export async function moderateReview(id, status) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            showToast(window.t('messages.review_moderated'));
        } else {
...
export async function deleteReview(id) {
    const confirmed = await window.showConfirmDialog({
        title: window.t('dialog.delete_review_title'),
        message: window.t('dialog.delete_review_msg'),
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            showToast(window.t('messages.review_deleted'));
        } else {
            showToast(window.t('messages.review_delete_failed') + ': ' + (data.message || ''));
            review._processing = false;
        }
    } catch (ex) {
        console.error(ex);
