...
if ($action ==='submit') {
    ...
    // 1. تسجيل الطلب
    $newRequest = ["id"=> $newId,"student_name"=> $studentName,"student_phone"=> $studentPhone,"student_info"=>"الجامعة:". $studentUni ."| الجنسية: أخرى","type"=> $serviceTitle,"details"=> $details,"status"=>"قيد المراجعة"];
    ...
    if (!empty($studentPhone)) {
        ...
        if (!$chatFound) {
            ...
            $newChat = ["id"=> $newChatId,"student_name"=> $studentName,"student_uni"=> $studentUni,"phone"=> $studentPhone,"last_msg"=> $replyText,"status"=>'طلب جديد',"time"=> $now,"messages"=> [
    ...
    echo json_encode(["status"=>"success","message"=>"تم استلام طلبك بنجاح وجاري المراجعة","request_id"=> $newId
