...
function performRegistration($conn, $fullName, $email, $phone, $password, $uni, $nationality = null) {
    ...
    try {
        // 2. Persistent Identity Blocklist Enforcement
        $blocked = isIdentityBlocked($conn, $email, $phone);
        if ($blocked) {
            $reasonMsg = !empty($blocked['reason']) ? " (السبب: {$blocked['reason']})" : "";
            return [
                "success" => false,
                "message" => "هذا الحساب أو المعرف محظور من إنشاء حساب جديد{$reasonMsg} / This identifier is blocked from registration.",
                "code" => 403
            ];
        }
        ...
        // 5. Insert Student
        $insertQuery = "INSERT INTO students (full_name, email, phone, password, university, nationality, points, is_blocked) VALUES (:full_name, :email, :phone, :password, :university, :nationality, 0, 0)";
