...
try {
    $stmt = $conn->prepare("SELECT id, full_name, email, phone, university, avatar_url, points AS points_balance, created_at FROM students WHERE id = ? LIMIT 1");
    $stmt->execute([$studentId]);
    $student = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($student) {
        // Ensure points_balance is integer type
        $student['points_balance'] = (int)$student['points_balance'];
