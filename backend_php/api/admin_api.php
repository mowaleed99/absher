...
try {
    if ($action === 'get_all') {
        ...
        echo json_encode([
            ...
            "stats" => [
                ...
                "pending_requests" => count(array_filter($requests, fn($r) => $r['status'] === 'قيد المراجعة'))
            ],
    ...
    if ($action === 'add_apartment') {
        ...
        if (!empty($title) && !empty($price)) {
            ...
            $stmtNotif->execute([
                "🏠 شقة سكنية جديدة معروضة للإيجار",
    ...
    if ($action === 'add_service') {
        ...
        if (!empty($title)) {
            ...
            $stmtNotif = $conn->prepare("INSERT INTO notifications (title, content, created_at) VALUES (?, ?, NOW())");
            $stmtNotif->execute([
                "🛠️ خدمة طلابية جديدة متوفرة الآن",
                "تمت إضافة خدمة طلابية جديدة: " . $title . ". تصفح قسم الخدمات للطلب والاستفسار مباشرة."
            ]);
